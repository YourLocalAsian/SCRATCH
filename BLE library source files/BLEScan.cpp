...
BLEScan::BLEScan() {
	...
	m_scan_params.scan_duplicate     = BLE_SCAN_DUPLICATE_DISABLE /*MENA BLE_SCAN_DUPLICATE_ENABLE*/;
		...
		case ESP_GAP_BLE_SCAN_RESULT_EVT: {
			...
			switch(param->scan_rst.search_evt) {
				...
				case ESP_GAP_SEARCH_INQ_CMPL_EVT: {
					log_v("ESP_GAP_SEARCH_INQ_CMPL_EVT");
					m_stopped = true;
				...
				case ESP_GAP_SEARCH_INQ_RES_EVT: {
					if (m_stopped) { // If we are not scanning, nothing to do with the extra results.
						//log_v("What the heck, how are we scanning when stopped???");
						break;
					}
                    log_v("Scan result, not stopped"); 
...
void BLEScan::setAdvertisedDeviceCallbacks(BLEAdvertisedDeviceCallbacks* pAdvertisedDeviceCallbacks, bool wantDuplicates, bool shouldParse) {
	...
	if(m_pAdvertisedDeviceCallbacks)
	    log_i("Advertised callback function has been successfully set!");
...
bool BLEScan::start(uint32_t duration, void (*scanCompleteCB)(BLEScanResults), bool is_continue) {
    ...
    log_v("The errRC is %s", errRc == ESP_OK? "GOOD":"BAD!");
	if (errRc != ESP_OK) {
	...
	if(bt_state = ESP_BLUEDROID_STATUS_ENABLED)
	    log_v("Bluedroid is GOOD");
	else 
	    log_v("Bluedroid is BAD");

	errRc = ::esp_ble_gap_start_scanning(duration);
    if(errRc == ESP_OK)
	{
		log_v("This shoudnt be okay. ");
	}
	if (errRc != ESP_OK) {
...
BLEScanResults BLEScan::start(uint32_t duration, bool is_continue) {
	log_v("The duration is %d, is_continue is %d", duration, is_continue);
	if(start(duration, nullptr, is_continue)) {
		//m_semaphoreScanEnd.wait("start");   // Wait for the semaphore to release.
	log_v("wazzup");
	}
	log_v("AFTER IF The duration is %d, is_continue is %d", duration, is_continue);
...
void BLEScan::erase(BLEAddress address) {
	...
	log_i("test 1");
	if(! advertisedDevice)
	    log_i("ADVERTISED DEVICE IS NULL!!");
    else
	    log_i("ADVERTISED DEVICE IS NOT NULL");
	m_scanResults.m_vectorAdvertisedDevices.erase(address.toString());
	log_i("test 2");
	//delete advertisedDevice; MENA COMMENTED THIS OUT. apparently causes error
	log_i("test 3");
}
