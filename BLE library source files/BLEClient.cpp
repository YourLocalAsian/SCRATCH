...
// and then block on its completion.  When the event has arrived, we will have the handle.
	...
	m_semaphoreRegEvt.take("connect");
	 ...
	 clearServices(); // we dont need to delete services since every client is unique?//SATURDAY
	...
	uint32_t rc = m_semaphoreRegEvt.wait("connect"); //SATURDAY tried commenting this out, maybe semaphore blocks events? TODO 
...
void BLEClient::gattClientEventHandler(
	...
	switch(event) {
		...
		case ESP_GATTC_DISCONNECT_EVT: {
				...
				m_isConnected = false;
				log_v("Before claering");
				clearServices();//MENA added this again SATURDAY
				log_v("after clearing");
				esp_ble_gattc_app_unregister(m_gattc_if); 
				m_gattc_if = ESP_GATT_IF_NONE; 
				m_semaphoreOpenEvt.give(ESP_GATT_IF_NONE);
				m_semaphoreRssiCmplEvt.give();
				m_semaphoreSearchCmplEvt.give(1);
			    log_v("Before removing");
			    BLEDevice::removePeerDevice(m_appId, true); 
			   log_v("After removing");
				if (m_wasConnected && m_pClientCallbacks != nullptr) {
					m_pClientCallbacks->onDisconnect(this);
				}
				//m_semaphoreOpenEvt.give(ESP_GATT_CANCEL); //MENA
				break;
		} // ESP_GATTC_DISCONNECT_EVT
