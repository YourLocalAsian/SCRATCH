...
void BLECharacteristic::notify(bool is_notification) {
	...
	//GeneralUtils::hexDump((uint8_t*)m_value.getValue().data(), m_value.getValue().length()); MENA commented out as its useless except for debug
...
void BLECharacteristic::setValue(uint8_t* data, size_t length) {
	...
	//free(pHex); MENA commented out these three lines as the mallocing and freeing take up time for nothing
...
void BLECharacteristic::setValue(uint64_t& data64) {
	uint8_t temp[8];
	temp[0] = data64;
	temp[1] = data64 >> 8;
	temp[2] = data64 >> 16;
...
void BLECharacteristic::setValue(uint64_t& data64, uint64_t& data64_2) {
	uint8_t temp[16];
	temp[0] = data64;
	temp[1] = data64 >> 8;
	temp[2] = data64 >> 16;
