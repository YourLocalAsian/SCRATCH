    ...
    //non blind mode: camera and display
        ...
        if(send_picture){
          ...
          //take and send picture
                ...
                myCAM.start_capture();
                delay(100);
                int start_capture_time = millis(); 
                while (!myCAM.get_bit(ARDUCHIP_TRIG, CAP_DONE_MASK)){
                    if (millis() - start_capture_time > 2000){
                        Serial.println("Capturing freeze... exiting!");  //Will the camera fix itself? or do we need to initialize? or do we have to turn off power?
                         myCAM.flush_fifo();
                        delay(100);
                        myCAM.clear_fifo_flag();
                        break;
